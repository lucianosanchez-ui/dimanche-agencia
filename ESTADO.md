...
**Falta para que PRODUZCA en serio por Telegram (= "full listo"):** (1) **bot-productor** [arriba]; (2) **Calendario de Notion limpio** — BLOQUEADO hoy: la DB no tiene estado "Archivado" ni la tool expone papelera → resolver agregando opción "Archivado" o vaciándolo desde la UI; (3) bajar la **regla de oro a Notion** (REF-002/030 + `brand/20-brand-visual-kit.md`) — el bot ya la tiene; (4) menor: **dedup de ~36 entradas duplicadas de Competencia** en *Inteligencia & Ideas* (corrida larga de Vera). Luciano NO quiere lanzar a Anto a producir hasta cerrar esto.
